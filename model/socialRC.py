...
def test_recommendation(user_rep, item_rep, score_model, test_path, neg_test_path):
    # read train_embedding and test_node(user/item)
    ...
    k = [5, 10, 15, 20, 25, 30, 35, 40, 45, 50] # topK (list)

    ap_vector = np.zeros([len(test_user_list), len(k)]) # AP
    recall_vector = np.zeros([len(test_user_list), len(k)]) # Recall
    ndcg_vector = np.zeros([len(test_user_list), len(k)]) # NDCG
    ...
    mean_ap = np.mean(ap_vector, axis=0) # MAP
    mean_recall = np.mean(recall_vector, axis=0) # Mean-Recall
    mean_ndcg = np.mean(ndcg_vector, axis=0) # Mean-ndcg
    ...
    #     f.write("the Map at :{}\n".format(mean_ap))
    #     f.write("the mean recall at :{}\n".format(mean_recall))
    #     f.write("the mean ndcg at :{}\n".format(mean_ndcg))
    #     f.write("----------------------------------\n")
