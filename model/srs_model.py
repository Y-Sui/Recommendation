...
class NetworkRS(object):
    def __init__(self, user_size, item_size, embed_len, score_model,
    ...
    def _build_user_model(self):
        ...
        if self.user_mask is not None:
            target_embedding = MultiMask(self.user_size+1, self.user_mask)([target_input, target_embedding])  # MultiMask;

        first_node_input = Input(shape=(1,), dtype='int32', name='first_input_target')  # first-order user = u_{t,m}
        second_node_input = Input(shape=(self.second_dims,), dtype='int32', name='second_input_target')  # second-order user = u_{t,m,k}
        first_embedded_node = self.user_embed(first_node_input)  # first-order influential neighbors = e_{t,m}
        second_embedded_nodes = self.user_embed(second_node_input)  # second-order influential neighbors = e_{t,m,k}

        # Second level ICAUs: generating the ICE r_{t,m} formula (6,7,8)
        ...
        # Subsidiary influence embedding c_{t,m} : a_{t,m,k}, e_{t,m} formula(9)
        ...
        first_memory_model = Model(inputs=[target_input, first_input, second_input], outputs=attention_first,
                                   name='first_mem_model')

        # formula(11) the influence gate model
