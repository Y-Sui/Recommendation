...
# item_net_path = '../datasets/%s/%s_itemNet.txt' % (data_name, data_name)
#
...
#
# item_rep_path = "../datasets/%s/%s_item_rep_user_cold.txt" % (data_name, data_name) # save the representation of the item
# user_rep_path = "../datasets/%s/%s_user_rep_user_cold.txt" % (data_name, data_name) # save the representation of the user

# neg_test_path= "../datasets/%s/%s_rating_test_cold_item_neg.txt" % (data_name, data_name)
#
# train_path = "../datasets/%s/%s_rating_train_cold_item.txt" % (data_name, data_name)
# test_path = "../datasets/%s/%s_rating_test_cold_item.txt" % (data_name, data_name)
#
# item_rep_path = "../datasets/%s/%s_item_rep_item_cold.txt" % (data_name, data_name)
# user_rep_path = "../datasets/%s/%s_user_rep_item_cold.txt" % (data_name, data_name)

user_net_path = '../datasets/movie/userNet.txt'
item_net_path = '../datasets/movie/itemNet.txt'
train_path = '../datasets/movie/train.txt'
test_path = '../datasets/movie/test.txt'
item_rep_path = '../datasets/movie/item_rep.txt'
user_rep_path = '../datasets/movie/user_rep.txt'

# neg_test_path = '../datasets/lastfm/lastfm_rating_test_cold_user_neg.txt'

neg_test_path = '../datasets/movie/neg.txt'
data_name = 'movie'
...
test_data = np.loadtxt(test_path, dtype=np.int32) # lastfm_rating_test_cold_user.txt

G_ui = np.loadtxt(train_path, dtype=np.int32) # lastfm_rating_train_cold_user.txt

test_user_list = list(set(test_data[:, 0])) # test_user_list [4,16,17,18,...]

print(G_ui)
print(test_user_list)

G_user = read_graph(user_net_path) # model.graph_utilities.read_graph -> networkx.read_edgelist from lastfm_userNet.txt
G_item = read_graph(item_net_path) # model.graph_utilities.read_graph -> networkx.read_edgelist from lastfm_itemNet.txt
...
edges = G_ui # interactions between users and items, reflected as the edges of the net.
...
batch_num = math.ceil(num_edges / batch_size) # return the ceiling of x as an Integral which is the smallest integer >= x.

loss = max_margin_loss # loss function

# score_model = nn_scoremodel((embed_len,), embed_len, score_act=None)
score_model = inner_prod_scoremodel((embed_len,), score_rep_norm=False) # score_model -> core tech
# score_model = fm_scoremodel((embed_len,), score_rep_norm=False, score_act=None)


# -----------------------------------------
# ------ Pretrained Model(Model_1) --------
# ---------- contrastive_model ------------
# -----------------------------------------
pretrain_model = mlmf(nb_user=user_size + 1, nb_item=item_size + 1, embed_dim=embed_len,
                      score_model=score_model, reg=l2(1e-7)) # model.mlmf component
pretrain_model.contrast_model.compile(loss=loss, optimizer='adam')
pretrain_model.contrast_model.summary()

pretrain_samples = 3
pretrain_batch_sz = 200
pretrain_batch_num = math.ceil(num_edges / pretrain_batch_sz) # same as the batch_num above
pretrain_iter = 3
...
for i in range(pretrain_iter):
    ...
    for s in range(pretrain_samples):
        for j in range(pretrain_batch_num):
            edge_batch = np.array(edges[j * pretrain_batch_sz:min(num_edges, (j + 1) * pretrain_batch_sz)]) # obtain the edge_batch
            batch_node_array, positive_batch_array, negative_batch_array = \
                (edge_batch[:, 0], edge_batch[:, 1], np.random.randint(low=1, high=item_size, size=len(edge_batch))) # negative_batch_array->noise
...
for i in range(max_iter):
    edges = shuffle(edges) # random shuffle the edges
    train_loss = 0
    # print("Running on iteration %d/%d:"%(i, max_iter))
    ...
    batchGenerator.clear_node_cache() # remove all items in D
