...
data_name = 'lastfm'
user_net_path = '../datasets/%s/%s_userNet.txt' % (data_name, data_name)  # user-net path
item_net_path = '../datasets/%s/%s_itemNet.txt' % (data_name, data_name)  # item-net path
ui_net_path = '../datasets/%s/%s_rating.txt' % (data_name, data_name)  # User-item-net path

user_neg_test_path = "../datasets/%s/%s_rating_test_cold_user_neg.txt" % (data_name, data_name)
item_neg_test_path = "../datasets/%s/%s_rating_test_cold_item_neg.txt" % (data_name, data_name)
user_train_path = "../datasets/%s/%s_rating_train_cold_user.txt" % (data_name, data_name)
item_train_path = "../datasets/%s/%s_rating_train_cold_item.txt" % (data_name, data_name)
user_test_path = "../datasets/%s/%s_rating_test_cold_user.txt" % (data_name, data_name)
item_test_path = "../datasets/%s/%s_rating_test_cold_item.txt" % (data_name, data_name)

user_rep_path = "../datasets/%s/%s_user_rep_user_cold.txt" % (data_name, data_name)  # save user_rep, not load!
item_rep_path = "../datasets/%s/%s_item_rep_user_cold.txt" % (data_name, data_name)  # save item_rep, not load!
...
def get_item_rep(model, G_item, embed_len, item_rep_path, batch_size=100, save=False):
    ...
    for j in range(nb_batch):
        ...
        first_batch_data, _ = batchGenerator.itemGenerate.get_batch_data_topk(batch_node=batch_node, topK=topK,
                                                                              predict_batch_size=100, order=1)
...
user_test_data = np.loadtxt(user_test_path, dtype=np.int32)  # lastfm_rating_test_cold_user.txt
user_test_list = list(set(user_test_data[:, 0]))  # convert np.int32 to list type
item_test_data = np.loadtxt(item_test_path, dtype=np.int32)  # lastfm_rating_test_cold_item.txt
item_test_list = list(set(item_test_data[:, 0]))

G_user = read_graph(user_net_path)  # networkx.read_edgelist; lastfm_userNet.txt
G_item = read_graph(item_net_path)  # networkx.read_edgelist; lastfm_itemNet.txt
G_ui = np.loadtxt(user_train_path, dtype=np.int32)  # lastfm_rating_train_cold_user.txt

directed = False
user_list = list(G_user.nodes())
item_list = list(G_item.nodes())

user_size = len(user_list)  # 1892
item_size = len(item_list)  # 12358
edges = G_ui  # interactional edges of the user_item_Net
num_edges = len(edges)  # 68691
...
batch_num = math.ceil(num_edges / batch_size)  # return the ceiling of the integral of num_edges / batch_size

loss = max_margin_loss  # loss_function

# score_model = nn_scoremodel((embed_len,), embed_len, score_act=None)
score_model = inner_prod_scoremodel((embed_len,), score_rep_norm=False)  # define the score model
...
for i in range(pretrain_iter):
    shuffle(edges)  # shuffle the data using **sklearn.utils.shuffle**
    train_loss = 0
    print("Running on iteration %d/%d:" % (i, max_iter))
    for s in range(pretrain_samples):
        for j in range(pretrain_batch_num):
            edge_batch = np.array(
                edges[j * pretrain_batch_sz:min(num_edges, (j + 1) * pretrain_batch_sz)])  # obtain each edge_batch
        ...
        # with open('../output/recording.txt', 'a', encoding='utf-8') as f:
        #     f.write("Training on sample %d and iter %d\n" % (s + 1, i + 1))
    print("Finish iteration %d/%d with loss: %f" % (
    i + 1, pretrain_iter, train_loss / (pretrain_batch_num * pretrain_samples)))
    # with open('../output/recording.txt', 'a', encoding='utf-8') as f:
    #     f.write("Finish iteration %d/%d with loss: %f\n" % (i + 1, pretrain_iter, train_loss / (pretrain_batch_num * pretrain_samples)))
    user_rep = pretrain_model.user_emb.get_weights()[0][1:]
    item_rep = pretrain_model.item_emb.get_weights()[0][1:]
    test_recommendation(user_rep, item_rep, pretrain_model.score_model, user_test_path, user_neg_test_path)
    # test_recommendation(item_rep, user_rep, test_path, neg_test_path) #for cold start item

model = NetworkRS(user_size, item_size, embed_len, score_model, topK, topK, embed_regularizer=l2(5e-7),
                  directed=directed, mem_filt_alpha=fliter_theta, mem_agg_alpha=aggre_theta, user_mask=None)
...
for i in range(max_iter):
    ...
    for s in range(spl):
        for j in range(batch_num):
            ...
            positive_first_batch, negative_first_batch = \
            ...
            if (j + 1) % 100 == 0:
                print("Training on batch %d/%d sample %d and iter %d on dataset %s" % (
                j + 1, batch_num, s + 1, i + 1, data_name))
                # with open('../output/recording.txt', 'a', encoding='utf-8') as f:
                #     f.write("Training on batch %d/%d sample %d and iter %d on dataset %s\n" % (j + 1, batch_num, s + 1, i + 1, data_name))
    print("Finish iteration %d/%d with loss: %f" % (i + 1, max_iter, train_loss / (batch_num * spl)))
    # with open('../output/recording.txt', 'a', encoding='utf-8') as f:
    #     f.write("Finish iteration %d/%d with loss: %f\n" % (i + 1, max_iter, train_loss / (batch_num * spl)))
    ...
    test_recommendation(user_rep, item_rep, model.score_model, user_test_path, user_neg_test_path)
    # test_recommendation(item_rep, user_rep, test_path, neg_test_path)  # for cold start item
